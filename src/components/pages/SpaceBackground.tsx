			...
			<div className="absolute inset-0 opacity-90">
				{planetCells.map(({ key, left, top }) => (
					...
					>
						<div className="relative" style={{ transform: "translate(-50%, -50%)" }}>
							<Planet
								weight="fill"
								style={{ width: `${PLANET_ICON_SIZE_PX}px`, height: `${PLANET_ICON_SIZE_PX}px` }}
							/>
							<div
				...
				{starCells.map(({ key, left, top }) => (
					...
					>
						...
						>
							<Star
								weight="fill"
								style={{ width: `${STAR_ICON_SIZE_PX}px`, height: `${STAR_ICON_SIZE_PX}px` }}
							/>
						</div>
					</div>
				))}
				...
				{SPACESHIP_LANES.map((leftPercent, index) => (
					...
					>
						<Rocket
							weight="fill"
							style={{ width: `${SPACESHIP_ICON_SIZE_PX}px`, height: `${SPACESHIP_ICON_SIZE_PX}px` }}
						/>
					</div>
				))}
			</div>
